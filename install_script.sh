...
if [ $OS == "MAC" ]; then
    ...
    sudo yaourt -S --aur --noconfirm --force tmux feh rofi neovim shellcheck python-pip zsh zsh-completions go python-neovim postgresql mariadb compton xorg-xbacklight hub redis tidy-html5 rxvt-unicode
    yaourt -S --aur --noconfirm --force nextcloud-client
    #sudo mysql_secure_installation
fi
